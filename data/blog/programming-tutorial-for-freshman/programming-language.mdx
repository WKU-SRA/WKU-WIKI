...
<div>
  java 代码中的`{}
  `为一个代码块，起到组织代码的作用，相当于python中的四个空格（`tab`）。注：在python规范中，
  请不要用`tab`替代四个空格。
</div>
...
有人说OOP完全是**过度设计（Overengineering）**，OOP的原则和设计模式鼓励将问题分解为对象和类，但是捏，这有时会导致过度设计。开发者可能会创建过多的类和继承层次，使代码变得复杂和难以维护，有时候我们完全不用类来实现业务逻辑，
比如爬取某个学校(在学校的允许范围内，此处无任何特指，我们不鼓励在未得到授权的情况下爬取他人网站)教授的科研方向，
